...
CREATE TABLE IF NOT EXISTS `school_class_schema`.`student_has_course` (
  ...
  PRIMARY KEY (`student_student_id`, `course_course_id`, `course_professor_professor_id`, `grade_grade_id`),
  ...
  INDEX `fk_student_has_course_grade1_idx` (`grade_grade_id` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_student_has_course_course1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_student_has_course_grade1`
    FOREIGN KEY (`grade_grade_id`)
    REFERENCES `school_class_schema`.`grade` (`grade_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;